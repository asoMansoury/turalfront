...
function CategoryModalDeleteGroup(props) {
    ...
    const notifyInfo = () => toast('در حال حذف گروهی موجودی اولیه ها ...', { duration: toastConfig.duration, style: toastConfig.infoStyle });
    const deleteBtn = () => {
        let input = props.selected.map(x => parseInt(x));
        axios.put(InitialBalanceRemoveByIdsApi, { ids: input })
            .then(res => {
                ...
                <Modal.Body>
                    {
                        props.selected.length == 1 ?
                            (<span className='description-modal'>
                                {props.selected.length} موجودی اولیه حذف میشود. تایید میکنید؟
                            </span>)
                            :
                            (<span className='description-modal'>
                                {props.selected.length} موجودی اولیه حذف میشوند. تایید میکنید؟
                            </span>)
                    }
                </Modal.Body>
                <Modal.Footer>
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(CategoryModalDeleteGroup,axios));