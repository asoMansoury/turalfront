...
import CostCategoryModalDelete from './CostCategoryModalDelete';
import CostCategoryRemoveByIds from './CostCategoryModalDeleteGroup';
import {CostCategorySearchApi} from '../commonConstants/ApiConstants'
import CostCategoryModalEdit from './CostCategoryModalEdit';
  ...
  return (
    ...
    >
        ...
        {numSelected > 0 ? (
          <Typography color="inherit" variant="subtitle1">
            ...
            <Typography variant="h6" id="tableTitle">
              <span className='tabelhead'>لیست سرفصل هزینه ها</span>
            </Typography>
          )}
      </div>
      ...
      <div className={classes.actions}>
        {numSelected > 0 ? (
         actionsPermission.find(z=>z.id==31)!==undefined? <CostCategoryRemoveByIds selected={props.selected} />:<></>
        )
          :
          (actionsPermission.find(z=>z.id==28)!==undefined?<Button onClick={addCategoryShow} className='btn-height2 create-btn' variant="info">افزودن دسته بندی</Button>:<></>)}
      </div>
    </Toolbar>
  );
};
...
export function CostCategoryList(props) {
  ...
  const getData = (nRows, page, title, inSearch, iseditingORdeletingORadding, isdeletinggroup) => {
    axios.get(CostCategorySearchApi + '?Title=' + title + '&ParentCode=0003&Page=' + page + '&Row=' + nRows+'&sort=date:desc')
      .then(res => {
        let result = res.data.costCategoryDtos;
      ...
      controllerPersmission!=undefined&&actionsPermission!=undefined?
        ...
        <ThemeProvider theme={theme}>
            <Paper className={classes.paper}>
              ...
              {
                actionsPermission.find(z=>z.id==30)!==undefined?
                          ...
                          <>
                            ...
                            >
                              ...
                              <TableCell className={classes.cell_short} align="center">
                                <Skeleton duration={1} style={{ width: '100%', display: isLoading ? 'block' : 'none', height: '20px' }} />
                                <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                  {row.description === '' ? <span className='pointer label label-lg label-light-danger label-inline btn-height'>فاقد توضیحات</span> : <CostCategoryModalEdit dbid={row.id} title="مشاهده توضیحات" headerTitle="دسته بند" name={row.title} text={row.description} />}
                                </div>
                              </TableCell>
                              <TableCell className={classes.cell_short} align="center">
                                <div style={{ display: !isLoading ? 'block' : 'none' }}>
                              ...
                              <TableCell className={classes.cell_short} align="center">
                                ...
                                <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                  {
                                    actionsPermission.find(z=>z.id==29)!==undefined?
                                  ...
                                  {
                                    actionsPermission.find(z=>z.id==31)!==undefined?
                                      <CostCategoryModalDelete dbid={row.id} name={row.title} />
                                    :<></>
                                  }
                                </div>
                              </TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(CostCategoryList,axios));