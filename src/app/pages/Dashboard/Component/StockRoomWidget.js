...
import {DashboardGetLastStockActivityApi} from '../../commonConstants/ApiConstants';
export function StockRoomWidget({ className }) {
  const [dto,setDto]=React.useState();

  useEffect(()=>{
    setTimeout(() => {
      axios.get(DashboardGetLastStockActivityApi).then((response)=>{
          let result = response.data;
          if(result.hasError==false){
            setDto(result.stockRoomDtos)
          }else{
    ...
    <>
      <div className={`card card-custom ${className}`}>
        ...
        <div className="card-header border-0">
          <h3 className="card-title font-weight-bolder text-dark">آخرین فعالیت ها در بخش مدیریت انبار</h3>
        </div>
        {/* Body */}
        {
          dto!=undefined?
...
export default checkRequests(StockRoomWidget,axios);