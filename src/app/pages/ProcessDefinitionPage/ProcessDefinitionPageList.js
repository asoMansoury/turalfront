...
import ProcessDefinitionModalDelete from './ProcessDefinitionModalDelete';
...
import {processDefinitionSearchApi} from '../commonConstants/ApiConstants';
import { Show_add, Show_edit, Is_not_edited, Is_not_deleted_one, Is_not_deleted_group, Is_not_added } from '../_redux/Actions/processDefinitionActions';
import { useDispatch, useSelector } from "react-redux";
import ProcessDefinitionModalDeleteGroup from './ProcessDefinitionModalDeleteGroup';
...
const headRows = [
    ...
    { id: 'Title', numeric: false, disablePadding: true, label: 'نام فرایند' },
          ...
          controllerPersmission!=undefined&&actionsPermission!=undefined?
              ...
              >
                  ...
                  {numSelected > 0 ? (
                    <Typography color="inherit" variant="subtitle1">
                      ...
                      <Typography variant="h6" id="tableTitle">
                        <span className='tabelhead'>لیست انواع فرایند</span>
                      </Typography>
                    )}
                </div>
                ...
                <div className={classes.actions}>
                  {numSelected > 0 ? (
                   actionsPermission.find(z=>z.id==82)!==undefined? <ProcessDefinitionModalDeleteGroup selected={props.selected} />:<></>
                  )
                    :
                    (actionsPermission.find(z=>z.id==81)!==undefined? <Button onClick={addCategoryShow} className='btn-height2 create-btn' variant="info">افزودن نوع فرایند</Button>:<></>)}
                </div>
              </Toolbar>
  ...
  export function ProcessDefinitionPageList(props) {
    ...
    const controllerPersmission = useSelector(state=>state.tokenReducer.TokenObject.userInfo.controllerDtos);
    const actionsPermission =useSelector(state=>state.tokenReducer.TokenObject.userInfo.actionDtos);
    function createData(id,title,code, description,createdDate) {
      return {code,title, createdDate,description,id  };
    }
    ...
    function editShowSlider(e) {
      ...
      dispatch(Show_edit({id:id,title:title,description:desc}));
    }
    ...
    const getData = (nRows, page, title, inSearch, iseditingORdeletingORadding, isdeletinggroup) => {
      let generateApi = processDefinitionSearchApi+'?Title=' + title +'&Page=' + page + '&Row=' + nRows+'&sort=date:desc';
      axios.get(generateApi)
        .then(res=>{
          let result = res.data.processDefinitionDtos;
        ...
        <div className={classes.root}>
            <ThemeProvider theme={theme}>
              <Paper className={classes.paper}>
                ...
                {
                    actionsPermission.find(z=>z.id==80)!==undefined?
                            ...
                            <>
                              ...
                              >
                                ...
                                <TableCell className={classes.cell_short} align="center">
                                  ...
                                  <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                    {
                                      actionsPermission.find(z=>z.id==79)!==undefined?
                                        <div className="delete-img-con btn-for-select" dbid={row.id}  title={row.title} description={row.description} onClick={editShowSlider} ><img className='edit-img btn-for-select' src={editImage} /></div>
                                      :<></>
                                    }
                                    {
                                      actionsPermission.find(z=>z.id==82)!==undefined?
                                        <ProcessDefinitionModalDelete dbid={row.id} name={row.title} />
                                      :<></>
                                    }
                                  </div>
                                </TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(ProcessDefinitionPageList,axios));