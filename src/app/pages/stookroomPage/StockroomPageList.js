...
import {stockRoomSearchApi,stockRoomRemoveByIds,stockRoomRemoveApi} from '../commonConstants/ApiConstants';
import { Show_add, Show_edit, Is_not_edited, Is_not_deleted_one, Is_not_deleted_group, Is_not_added } from '../_redux/Actions/stockRoomActions';
...
const headRows = [
    ...
    { id: 'Title', numeric: false, disablePadding: true, label: 'نام انبار' },
    ...
    { id: 'Address', numeric: false, disablePadding: true, label: 'آدرس' },
      ...
      >
          ...
          {numSelected > 0 ? (
            <Typography color="inherit" variant="subtitle1">
              ...
              <Typography variant="h6" id="tableTitle">
                <span className='tabelhead'>لیست انبارها</span>
              </Typography>
            )}
        </div>
        ...
        <div className={classes.actions}>
          {numSelected > 0 ? (
            ...
            actionsPermission.find(z=>z.id==87)!==undefined? <CategoryModalDeleteGroup selected={props.selected} />:<></>
          )
            :
            (actionsPermission.find(z=>z.id==84)!==undefined?<Button onClick={addCategoryShow} className='btn-height2 create-btn' variant="info">افزودن انبار</Button>:<></>)}
        </div>
      </Toolbar>
        :<></>
      }
  ...
  export function StockRoomPageList(props) {
    ...
    function createData(id,title,code, address,description,createdDate) {
      return {address,code,title, createdDate,description,id  };
    }
    ...
    function editShowSlider(e) {
      ...
      dispatch(Show_edit({id:id,title:title,description:desc,address:address}));
    }
    ...
    const getData = (nRows, page, title, inSearch, iseditingORdeletingORadding, isdeletinggroup) => {
      let generateApi = stockRoomSearchApi+'?Title=' + title +'&Page=' + page + '&Row=' + nRows+'&sort=date:desc';
      axios.get(generateApi)
        .then(res=>{
          let result = res.data.stockRoomDtos;
          ...
          result.forEach(item => {
            temp.push(createData(item.id,item.title,item.code,item.address,item.description == null ? '' : item.description,item.createdDate));
          });
          setRows(temp);
          setIsLoading(false);
      ...
      <>
      {
        ...
        <ThemeProvider theme={theme}>
          <Paper className={classes.paper}>
            ...
            {
              actionsPermission.find(z=>z.id==88)!==undefined?
                        ...
                        <>
                          ...
                          >
                            ...
                            <TableCell className={classes.cell_short} align="center">
                              <Skeleton duration={1} style={{ width: '100%', display: isLoading ? 'block' : 'none', height: '20px' }} />
                              <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                {row.address === '' ? <span className='pointer label label-lg label-light-danger label-inline btn-height'>فاقد آدرس</span> : <CategoryModalEdit dbid={row.id} title="مشاهده آدرس" name={row.title} text={row.address} />}
                              </div>
                            </TableCell>
                            <TableCell className={classes.cell_short} align="center">
                              <Skeleton duration={1} style={{ width: '100%', display: isLoading ? 'block' : 'none', height: '20px' }} />
                              <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                {
                                  actionsPermission.find(z=>z.id==87||z.id===89)!==undefined?
                                  <div className="delete-img-con btn-for-select" dbid={row.id} address={row.address} title={row.title} description={row.description} onClick={editShowSlider} ><img className='edit-img btn-for-select' src={editImage} /></div>
                                  :<></>
                                }
                                {
                                  actionsPermission.find(z=>z.id==83)!==undefined?
...
const mapStateToProps = (state => {
    return {
      Is_Edited: state.stockroomReducer.Is_Edited,
      Is_Deleted_One: state.stockroomReducer.Is_Deleted_One,
      Is_Deleted_Group: state.stockroomReducer.Is_Deleted_Group,
      Is_Added: state.stockroomReducer.Is_Added,
    };
});
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(StockRoomPageList,axios));