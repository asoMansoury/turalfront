...
import {stockRoomInsertApi} from '../commonConstants/ApiConstants';
import axios from 'axios';
import { Is_added } from '../_redux/Actions/stockRoomActions';
import checkRequests from '../component/ErrroHandling';
export class StockroomPageAdd extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    notifySuccess = (title) => toast('انبار ' + title + ' با موفقیت افزوده شد.', { duration: toastConfig.duration, style: toastConfig.successStyle });
    notifyError = (title) => toast(title , { duration: toastConfig.duration, style: toastConfig.errorStyle });
    notifyInfo = (title) => toast('در حال افزودن انبار  ' + title + ' ...', { duration: toastConfig.duration, style: toastConfig.infoStyle });
    ...
    save() {
        ...
        let address = this.addressInputRef.current.value;
        axios.post(stockRoomInsertApi, { Title: title, Description: desc,Address:address })
            .then(res => {
        ...
        return (
            <div className='category-add-container' style={{ left: this.state.leftSideBar + 'px', width: SideBarConfig.width + 'px' }}>
                ...
                <div className='category-add-body'>
                    ...
                    <Form.Control ref={this.addressInputRef} className='form-control-custom' as="textarea" rows="4" />
...
const mapStateToProps = (state => {
    return {
        Show_Hide_Add: state.stockroomReducer.Show_Hide_Add,
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(StockroomPageAdd,axios));