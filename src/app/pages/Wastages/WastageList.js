...
import {WastagePrintExcelApi,WastageSearchWastageApi} from '../commonConstants/ApiConstants';
...
const headRows = [
    ...
    { id: 'CategoryName', numeric: false, disablePadding: true, label: 'عنوان' },
    { id: 'Price', numeric: false, disablePadding: true, label: 'تعداد ضایعات' },
    { id: 'Price', numeric: false, disablePadding: true, label: 'واحد' },

    { id: 'Price', numeric: false, disablePadding: true, label: 'قیمت هر واحد(ریال)' },
  ];
            ...
            {numSelected > 0 ? (
              <Typography color="inherit" variant="subtitle1">
                ...
                <Typography variant="h6" id="tableTitle">
                  <span className='tabelhead'>لیست  ضایعات</span>
                </Typography>
              )}
          </div>
  ...
  export function WastageList(props) {
    ...
    function createData(id,unitName,wastageCount,title,pricePerUnit) {
      return {id, unitName,wastageCount,title,pricePerUnit};
    }
    ...
    const getData = (nRows, page, title, inSearch, iseditingORdeletingORadding, isdeletinggroup) => {
      axios.get(WastageSearchWastageApi+'?Title=' + title +'&Page=' + page + '&Row=' + nRows+'&sort=date:desc')
        .then(res=>{
          let result = res.data.wastageDtos;
          ...
          result.forEach(item => {
            temp.push(createData(item.id,item.unitName,item.wastageCount,item.title,item.pricePerUnit));
          });
          setRows(temp);
          setIsLoading(false);
      ...
      <>
      {
        ...
        <ThemeProvider theme={theme}>
          <Paper className={classes.paper}>
            <EnhancedTableToolbar numSelected={selected.length}    onAddClick={showAddSlider}/>
            {
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(WastageList,axios));