...
import FlowProcessModalDeleteGroup from './components/FlowProcessModalDeleteGroup';
import FlowProcessModalDelete from './components/FlowProcessModalDelete';
import Typography from '@material-ui/core/Typography';
import clsx from 'clsx';
import Toolbar from '@material-ui/core/Toolbar';
import { useHistory } from 'react-router-dom';
import {flowProcessEditPath,FlowProcessPDFPath,flowProcessAddPath} from '../commonConstants/RouteConstant';
...
const headRows = [
        ...
        <Toolbar
          ...
          >
              ...
              {numSelected > 0 ? (
                <Typography color="inherit" variant="subtitle1">
                  ...
                  <Typography variant="h6" id="tableTitle">
                    <span className='tabelhead'>لیست جریان فرایندها</span>
                  </Typography>
                )}
            </div>
            ...
            <div className={classes.actions}>
              {numSelected > 0 ? (
               actionsPermission.find(z=>z.id==39)!==undefined?  <FlowProcessModalDeleteGroup selected={props.selected} />:<></>
              )
                :
                (actionsPermission.find(z=>z.id==32||z.id==35)!==undefined?<Button onClick={addCategoryShow} className='btn-height2 create-btn' variant="info">افزودن خروجی فرایند</Button>:<></>)}
            </div>
          </Toolbar>
  ...
  export function FlowProcessList(props) {
    ...
    function editShowSlider(e) {
      ...
      history.push({
        pathname:flowProcessEditPath,
        state:{data:JSON.parse(row)}
      });
    }
    function printPdf(e){
      let id = e.currentTarget.getAttribute('dbid');
      localStorage.setItem("flowProcessData", id);
      window.open("/#/flowProcess/FlowProcessPDF", "_blank") //to open new page
    }
    ...
    const getData = (nRows, page, title, inSearch, iseditingORdeletingORadding, isdeletinggroup) => {
      let generateApi = FlowProcessSearchApi+'?Title=' + title +'&Page=' + page + '&Row=' + nRows+'&sort=date:desc';
        ...
        <div className={classes.root}>
          <ThemeProvider theme={theme}>
            <Paper className={classes.paper}>
              ...
              {
                actionsPermission.find(z=>z.id==37)!==undefined?
                          ...
                          <>
                            ...
                            >
                              ...
                              <TableCell className={classes.cell_short} align="center">
                                ...
                                <div style={{ display: !isLoading ? 'block' : 'none' }}>
                                  {
                                    actionsPermission.find(z=>z.id==36)!==undefined?
                                  ...
                                  {
                                    actionsPermission.find(z=>z.id==39)!==undefined?
                                      <FlowProcessModalDelete dbid={row.id} name={row.title} />
                                    :<></>
                                  }
                                </div>
                              </TableCell>
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(FlowProcessList,axios));