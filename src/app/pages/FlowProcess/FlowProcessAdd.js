...
export const FlowProcessAdd = (props) => {
  ...
  const totalWorkerPriceRef = useRef();
  const InitialBalanceAddRef = useRef();

  const [propsedPrice,setProposedPrice] = React.useState();
  ...
  const [selectedUnitItem,setSelectedUnitItem]=React.useState();
  const [selectedCategoryItem,setSelectedCategoryItem]=React.useState();
  const [selectedMaterialItem,setSelectedMaterialItem]=React.useState();
  const [isEnableMaterialDropDown,setIsEnableMaterialDropDown]=React.useState(true);
  ...
  const [unitTypeSource,setUnitTypeSource]=React.useState([]);
  const [categoryTypeArray,setCategoryTypeArray] = React.useState([]);
  ...
  const hireRialRef = useRef();

  useEffect(()=>{
  },props);
  useEffect(() => {
    hireRef.current.value=0;
    fillDropDownsData();
    fillProcess();
    fillFlowProcess();
    fillContractorType();
  ...
  useEffect(()=>{
    if(showNewBalance){
     ...
     ></InitialBalanceEdit>)
      ...
      setInitialBalanceRender(
      <InitialBalanceAdd
         balanceAddData={balanceAddData}
          ref ={InitialBalanceAddRef}
          isEnableMaterialDropDown={isEnableMaterialDropDown}
          propsedPrice={propsedPrice}
          updateBalanceAddModel = {updateBalanceAddModel}
          selectedUnitItem={unitTypeSource[0]!=undefined?unitTypeSource[0].id:0}
  ...
  },[showNewBalance,propsedPrice]);

  useEffect(()=>{
    calculateTotalPrice();
  },[salaryData,processInfo]);
  useEffect(()=>{
    setSelectedUnitItem(unitTypeSource[0]!=undefined?unitTypeSource[0].id:0);
  },unitTypeSource);

  function fillDropDownsData(){
    let api= GeneralParamterGetChildsByParentsApi+"?Code="+unitTypeCode+"&Code="+categoryParentCode;
    ...
    axios.get(api).then((res)=>{
        if(res.data.hasError==false){
            ...
            setUnitTypeSource(unitTypesArray);
            setCategoryTypeArray(categoryTypeArray);
            
            setSelectedCategoryItem(categoryTypeArray[0]!=undefined?categoryTypeArray[0].id:0);
        }
    })
}
  function calculateTotalPrice(){
    if(processInfo!=undefined&&salaryData[0]!=undefined){
      let totalPriceProcess =parseInt(processInfo.processDto.totalPrice);
      let totalPriceWorkers = 0;
      var salaryWorkerPayment = 0;
      salaryData.map((item,index)=>{
        if(item.salaryTypeFK_ID==14){
            salaryWorkerPayment =salaryWorkerPayment+ (item.salary*item.SumHours);
        }//محاسبه دستمزد کارگر ساعتی
        
        else{
            salaryWorkerPayment =salaryWorkerPayment+ (parseInt(item.salary)*parseInt(item.SumHours));
        }
      });
      totalPriceWorkers = parseInt(totalPriceProcess) + Math.round(parseInt(salaryWorkerPayment));
      setProposedPrice(Math.round(totalPriceWorkers));
      totalWorkerPriceRef.current.value = NumberToWords.convert(Math.round(salaryWorkerPayment)) + ' ریال';
    }
  }
  ...
  function fillFlowProcess() {
    ...
    axios.get(api).then((res) => {
      if (res.data.hasError === false) {
        if(res.data.generalParamterDtos.length>0){
          res.data.generalParamterDtos.map((item, index) => {
            tmpArray.push(item);
          })
  ...
  function getProcessInformation(id){
      axios.get(ProcessGetByIDApi+'/'+id).then((res)=>{
        if(res.data.hasError===false){
          ...
          totalPriceRef.current.value =NumberToWords.convert(Math.round(res.data.processDto.totalPrice)) + ' ریال';
          setProposedPrice(res.data.processDto.totalPrice)
          setPageModel({
            ...pageModel,
          ...
          if(res.data.processDto.isFinalStep==true){
            setIsEnableMaterialDropDown('disabled');
            setSelectedMaterialItem(props.materialTypesSource[1]!=undefined?props.materialTypesSource[0].id:0);
          }else{
            setIsEnableMaterialDropDown('disabled');
            setSelectedMaterialItem(props.materialTypesSource[0]!=undefined?props.materialTypesSource[1].id:0);
          } 
        } 
      }).catch((error)=>{
  ...
  function processTypeChange(e){
    ...
    var flowProcessTypeID = flowProcessTypesSource[0]!=undefined?flowProcessTypesSource[0].id:0;
    setSelectedFlowProcessType(flowProcessTypeID);
    setFlowProcessChange(flowProcessTypeID)
  ...
  function hireOnKeyUp(e){
    var hire = parseInt(e.target.value);
    if(hire.toString()==='NaN'){
      hire=0;
    }
    if(hire.toString()==='')
      hire=0;
    hireRef.current.value = hire;
    hireRialRef.current.value = NumberToWords.convert(Math.round(hire)) + ' ریال'; 
    let totalPriceProcess =parseInt(processInfo.processDto.totalPrice);
    let totalPriceProposed = totalPriceProcess+hire;
    setProposedPrice(Math.round(totalPriceProposed));
  }
    ...
    if (showContractorType === false)
        ...
        <>
          <Col md='3'>
            ...
            <Form.Control style={{marginTop: '20px' }} 
              className={classes.inputBackGround, 'custom-label marg-t-20 bold'} placeholder="اجرت" 
              onKeyUp={hireOnKeyUp}
              ref={hireRef}></Form.Control>
          </Col>
          <Col md='5'>
              <Form.Control style={{marginTop:'45px',border: 'none',background: 'white'}} disabled='disabled'
               ref={hireRialRef}  type="Name" aria-required={true} />
          </Col>
        </>
      )
    else {
      return (
        <>
          ...
          <Col md='5'>
           <Form.Control disabled='disabled' style={{border: 'none',background: 'white',marginTop:'38px'}} disabled='disabled' className={classes.inputBackGround, 'custom-label marg-t-20 bold'}  ref={totalWorkerPriceRef}></Form.Control>
          </Col>
        </>
      )
    }
  ...
  const notifyInfo = (title) => toast('در حال افزودن خروجی فرایند  ' + title + ' ...', { duration: toastConfig.duration, style: toastConfig.infoStyle });
  ...
  function save() {
    ...
    if(showNewBalance===true){
      ...
      axios.post(FlowProcessInsertToStackApi,data).then((res)=>{
        ...
        if(res.data.hasError===false){
          saveWastage(res.data.flowProcessDto.id);
        }else
      ...
      var initialBalanceDto = {
        Title:props.balanceAddModel.Title,
        Description:props.balanceAddModel.Description,
        Price:props.balanceAddModel.Price,
        UnitMeaurementFK_ID:props.balanceAddModel.balanceModel.selectedUnitItem,
        TypeMaterialFK_ID:props.balanceAddModel.balanceModel.selectedMaterialItem,
        CategoryTypeFK_ID:props.balanceAddModel.balanceModel.selectedCategoryItem,
        StockRoom_InitialBalanceDto:props.balanceAddModel.StockRoom_InitialBalanceDto
      }
      var data ={
        Title:title,
        Description:description,
      ...
      formData.append("file", props.balanceAddModel.image);
  ...
  function saveWastage(flowProcessID){
    
    var data ={
      ID:flowProcessID,
      wastagesDto:wastageModel
    }
    axios.post(FlowProcessInsertWastageApi,data).then((res)=>{
      if(res.data.hasError===false){
              ...
              <Form style={{ border: '1px solid rgb(201, 211, 255)', padding: '21px' }}>
                <Form.Group>
                  ...
                  <Row >
                    ...
                    <Col md='6' >
                      <Form.Label className='custom-label bold'>هزینه مصرف مواد اولیه</Form.Label>
                      <Form.Control style={{marginTop:'38px',border: 'none',background: 'white'}} disabled='disabled'  
                      ref={totalPriceRef}></Form.Control>
                    </Col>
                  </Row>
...
const mapStateToProps = (state => {
  return {
    ...
    balanceAddModel:state.balance.BalanceModel
  };
});
...
export default connect(mapStateToProps, mapDispatchToProps)(checkRequests(FlowProcessAdd,axios));